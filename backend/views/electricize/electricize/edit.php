...
<div class="ibox-content">
    <form method="post" class="form-horizontal">
        ...
        <div class="form-group">
            ...
            <div class="col-sm-2">
                <input type="text" class="form-control" name="gunpoint" value="<?= $model->gunpoint ?>" readonly>
            </div>
        </div>
        <div class="hr-line-dashed"></div>
        <div class="form-group">
            ...
            <div class="col-sm-10 row">
                <?php for ($i = $model->gunpoint; $i > 0; $i--): ?>
                    <div class="col-sm-3
                     qrCode<?= $i ?>"></div>
                    <script>
                        $('.qrCode<?= $i ?>').makeCode({
            ...
            <div class="col-sm-2">
                <select class="form-control m-b" name="model_id">
                    <?php foreach (\vendor\en\Electricize::getModel() as $k => $v): ?>
                        <option value="<?= $k ?>" <?= $model->model_id == $k ? "selected" : "" ?>><?= $v ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
        </div>
            ...
            <div class="col-sm-2">
                <select class="form-control m-b" name="field_id">
                    <?php foreach (\vendor\en\Electricize::getField() as $k => $v): ?>
                        <option value="<?= $k ?>" <?= $model->field_id == $k ? "selected" : "" ?>><?= $v ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
        </div>
            ...
            <div class="col-sm-2">
                <select class="form-control m-b" name="section_id">
                    <?php foreach (\vendor\en\Electricize::getSection() as $k => $v): ?>
                        <option value="<?= $k ?>" <?= $model->section_id == $k ? "selected" : "" ?>><?= $v ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
        </div>

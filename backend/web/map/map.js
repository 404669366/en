//document.write('<script type="text/javascript" src="http://api.map.baidu.com/getscript?v=2.0&ak=EZPCgQ6zGu6hZSmXlRrUMTpr"></script>');
window.TILE_VERSION = {
    "ditu": {
        "normal": {"version": "088", "updateDate": "20180617"},
        "satellite": {"version": "009", "updateDate": "20180617"},
        "normalTraffic": {"version": "081", "updateDate": "20180617"},
        "satelliteTraffic": {"version": "083", "updateDate": "20180617"},
        "mapJS": {"version": "104", "updateDate": "20180617"},
        "satelliteStreet": {"version": "083", "updateDate": "20180617"},
        "panoClick": {"version": "1033", "updateDate": "20180607"},
        "panoUdt": {"version": "20180607", "updateDate": "20180607"},
        "panoSwfAPI": {"version": "20150123", "updateDate": "20150123"},
        "panoSwfPlace": {"version": "20141112", "updateDate": "20141112"},
        "earthVector": {"version": "001", "updateDate": "20180617"}
    },
    "webapp": {
        "high_normal": {"version": "001", "updateDate": "20180617"},
        "lower_normal": {"version": "002", "updateDate": "20180617"}
    },
    "api_for_mobile": {
        "vector": {"version": "002", "updateDate": "20180617"},
        "vectorIcon": {"version": "002", "updateDate": "20180617"}
    }
};
window.BMAP_AUTHENTIC_KEY = "EZPCgQ6zGu6hZSmXlRrUMTpr";
(function () {
    function ba(a) {
        throw a;
    }
...
function Map(config) {
    var big = 16;
    var id = config.id ? config.id : 'map';
    $('#map').css('width', config.width ? config.width : '100%');
    $('#map').css('height', config.height ? config.height : '800px');
    //创建地图
    var map = new BMap.Map(id, {enableMapClick: false});
    map.enableScrollWheelZoom();
    //绑定点击事件
    map.addEventListener("click", function (e) {
        addPoint(false, e.point);
    });
    //有默认使用默认没有获取当前定位
    if (config.default) {
        var po = new BMap.Point(config.default.lng, config.default.lat);
        addPoint(true, po);
    } else {
        var myCity = new BMap.LocalCity();
        myCity.get(function (result) {
            addPoint(true, result.center);
        });
    }

    function addPoint(need, cl) {
        map.clearOverlays();
        var point = new BMap.Point(cl.lng, cl.lat);
        if (need) {
            map.centerAndZoom(point, big);
        }
        //标注点
        ...
        geoc.getLocation(cl, function (rs) {
            //调用回调
            if (config.callBack) {
                config.callBack(doInfo(rs));
            }
        });
    }

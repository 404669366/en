...
function picWall(config) {
    ...
    $(config.element).css({
        ...
        'padding': '15px',
        'height': (config.height * times || 10 * times) + 'rem'
    });
    var images = '';
    config.image.split(',').forEach(function (v, k) {
        if (v) {
            images += '<img class="data-img" src="' + v + '" style="height: ' + Math.floor(100 / times) + '%; width: 33.33%">';
        }
    });
